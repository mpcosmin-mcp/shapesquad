/* ─────────────────────────────────────────────────────────
   ...
   no Node, no @upstash/redis) so both sides can use it.

   Back-compat: normalizeReactions handles any legacy bare string[]
   (old hearts only) by upgrading to { '❤️': arr }.
   ───────────────────────────────────────────────────────── */

/** The fixed emoji palette — body-comp themed: muscle · fire · love. */
export const REACTIONS = ['💪', '🔥', '❤️'] as const;
export type ReactionEmoji = (typeof REACTIONS)[number];

/** Default reaction — a bare "like" is ❤️. */
