...
/* ═══════════════════════════════════════════════════════════════════════
 * Social layer — likes + comments + Instagram-style 1-level reply threading.
 *
 * Source of truth: Vercel KV (Upstash Redis hashes `shape:likes`,
 * `shape:comments`). Documented in SOCIAL_SYNC.md.
 *
 * Lifecycle:
 *   1. Mount → hydrate from localStorage instantly.
 *   2. Refetch from KV in the background.
 *   3. Refetch on window focus + every 30s.
 *   4. Mutations are optimistic; we rollback ONLY on 4xx (real bad request).
 *      5xx / network / kv-unavailable keep the optimistic state — offline-first.
 *
 * entryKey = `${date}_${name}` — same convention as sleep tracker.
 * ═══════════════════════════════════════════════════════════════════════ */

const LIKES_KEY    = 'shapesquad_likes_v2';
const COMMENTS_KEY = 'shapesquad_comments_v3';
...
/** Pad legacy KV records (pre-threading) with default arrays. */
...
export function SocialProvider({ children }: { children: ReactNode }) {
  ...
  // Synchronous hydration on mount
  useEffect(() => {
  ...
  // Background refetch: on mount, on focus, every 30s
  useEffect(() => {
    let cancelled = false;
    const refresh = async () => {
      try {
  ...
  const mutateEntryComments = (entryKey: string, transform: (arr: Comment[]) => Comment[]) => {
    let snapshot: Comment[] = [];
    setComments(prev => {
      snapshot = prev[entryKey] ?? [];
  ...
  const handleMutationError = (entryKey: string, snapshot: Comment[], err: ApiError) => {
    if (err?.status && err.status >= 400 && err.status < 500) {
      setComments(curr => {
        const reverted: CommentsMap = { ...curr };
        if (snapshot.length === 0) delete reverted[entryKey];
...
/* ─── Hook with `likesFor` / `commentsFor` convenience ─────────── */
