/* ─────────────────────────────────────────────────────────
   Competition primitives — per-metric leaderboards.

   Pure functions over Person[] (domain logic, no React). The page in
   app/clasament/ renders these; social feedback (emoji reactions, comments)
   lives on the boards themselves, keyed by each person's latest entry
   (`${date}_${name}`) via <ReactionBar/> / <EntryReactions/>.
   ...
   ───────────────────────────────────────────────────────── */
import { type Person, type MetricKey, type MetricDef, METRICS } from './shape';
