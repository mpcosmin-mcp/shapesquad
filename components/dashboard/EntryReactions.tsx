 ...
 *
 * Instagram-style 1-level threading. Heart on each level (entry, comment, reply).
 *
...
export function EntryReactions({
  date, name, currentUser, allNames,
}: {
  date: string;
  name: string;
  currentUser: string;
  ...
  const key = entryKeyOf(date, name);
  ...
  return (
    <div className="mt-2.5 pt-2 border-t border-[var(--color-border)]/60">
      <div className="flex items-center gap-3">
        ...
        {entryLikes.length > 0 && (
          <span className="text-[10px] text-[var(--color-fg-faint)] truncate ml-auto">
            {entryLikes.slice(0, 3).map((u) => u.split(/\s+/)[0]).join(', ')}
...
/* ─── Comment block (top-level comment + reply thread) ───── */

function CommentBlock({
  comment, currentUser, allNames,
  onDelete, onToggleLike, onReply, onToggleReplyLike, onDeleteReply,
}: {
  comment: Comment;
  currentUser: string;
  allNames: string[];
